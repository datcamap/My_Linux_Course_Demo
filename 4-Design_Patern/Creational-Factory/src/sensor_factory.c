...
int sensor_register_driver(const SensorDriver* drv) {
    if (!drv || !drv->kind || !drv->create) {
        return -1;
    }
    if (g_num_drivers >= SENSOR_MAX_DRIVERS) {
        return -2;
    }

    for (size_t i = 0; i < g_num_drivers; ++i) {
        if (0 == strcmp(g_drivers[i]->kind, drv->kind)) {
            return -3;
        }
    }
...
Sensor* sensor_create(const char* kind, const SensorConfig* cfg) {
    for (size_t i = 0; i < g_num_drivers; ++i) {
        ...
        Sensor* sens = g_drivers[i]->create();
        if (!sens) {
            return NULL;
        }
        if (sensor_init(sens, cfg) == SENSOR_OK) {
            return sens;
        }
        sensor_destroy(sens);
        return NULL;
